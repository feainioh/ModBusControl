    ...
    public partial class PLCInterface : Frame
    {
        public PLCInterface()
        {
            ...
            foreach (TabPage item in this.tabControl_PLC.TabPages)
            {
                ...
                else Axis_CCD.Parent = tabControlNF_Axis;
            }
        }
        ...
        private void Show_Coord()
        {
            ...
            //扫描条码位置[2018.3.28 lqz]
            this.textBox_XScan.Text = ((double)GlobalVar.c_Modbus.HoldingRegisters.AxisDot_ScanPoint.Value / GlobalVar.ConverRate).ToString("#0.00");
            this.textBox_YScan.Text = ((double)GlobalVar.c_Modbus.HoldingRegisters.AxisY_ScanPoint.Value/ GlobalVar.ConverRate).ToString("#0.00");
            //相机拍照位置
            this.textBox_XCCD.Text = ((double)GlobalVar.c_Modbus.HoldingRegisters.CCD_TargetPoint.Value / 640).ToString("#0.00");
            this.textBox_YCCD.Text = ((double)GlobalVar.c_Modbus.HoldingRegisters.AxisY_PhotoLocation.Value/GlobalVar.ConverRate).ToString("#0.00");
            //原点
            this.textBox_Xorigin.Text = GlobalVar.gl_origin_X.ToString("#0.00");
            this.textBox_Yorigion.Text = GlobalVar.gl_origin_Y.ToString("#0.00");
        }
        ...
        private void either_FeedIsPress_Event_BtnClick(object sender, LeftRightSide lr)
        {
            switch (lr)
            {
